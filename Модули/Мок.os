...
Функция СоздатьМок(Знач ИсходныйОбъект, Знач ЭтоШпион = Ложь)
	...
	Для Каждого Метод Из Методы Цикл
		...
		Для Каждого ПараметрМетода Из Метод.Параметры Цикл
			...
			МассивАннотацийПараметраМетода = Новый Массив;
			Если АннотацииПоддерживаются 
				И НЕ ПараметрМетода.Аннотации = Неопределено Тогда
				МассивАннотацийПараметраМетода = ПараметрМетода.Аннотации.ВыгрузитьКолонку("Имя");
			КонецЕсли;	

			ПараметрыМетодаСАмперсандом = Новый Массив;
			Для Каждого АннотацияПараметра Из МассивАннотацийПараметраМетода Цикл
				ПараметрыМетодаСАмперсандом.Добавить(СтрШаблон(ШаблонАннотации, АннотацияПараметра));
			КонецЦикла;
			СтрокаАннотацийПараметров = СтрСоединить(ПараметрыМетодаСАмперсандом, Символы.ПС);
		...
		МассивАннотаций = Новый Массив;
		Если АннотацииПоддерживаются 
			И НЕ Метод.Аннотации = Неопределено Тогда
				МассивАннотаций = Метод.Аннотации.ВыгрузитьКолонку("Имя");
		КонецЕсли;	
		
		МассивСАмперсандом = Новый Массив;
		Для Каждого АннотацияМетода Из МассивАннотаций Цикл
			МассивСАмперсандом.Добавить(СтрШаблон(ШаблонАннотации, АннотацияМетода));
		КонецЦикла;
		СтрокаАннотацийМетода = СтрСоединить(МассивСАмперсандом, Символы.ПС);
