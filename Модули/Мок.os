...
Функция СоздатьМок(Знач ИсходныйОбъект, Знач ЭтоШпион = Ложь)
	...
	РефлекторФейковогоОбъекта = Новый Рефлектор;
	СвойстваФейковогоОбъекта = Рефлектор.ПолучитьТаблицуСвойств(ФейковыйОбъект);

	ИменаСвойств = Новый Массив;
	Для Каждого СтрокаСвойства Из СвойстваФейковогоОбъекта Цикл

		ИменаСвойств.Добавить(СтрШаблон("%1, ", СтрокаСвойства[0]));
	
	КонецЦикла;	
	
	ЗаполнитьЗначенияСвойств(ФейковыйОбъект, ИсходныйОбъект, СтрСоединить(ИменаСвойств));
